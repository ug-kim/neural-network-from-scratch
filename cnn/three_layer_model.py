...
class ThreeLayerNet():
    def __init__(self, input_dim=(1, 28, 28), filter_size=3, num_classes=10, scale=1e-1, reg=0.0):
        ...
        # (10, 7, 7) -> (16, 7, 7) -> (16, 3, 3)
        self.params['w_3'] = scale * \
            np.random.randn(num_filters_3, num_filters_2,
                            filter_size, filter_size)
        self.params['b_3'] = np.zeros(num_filters_3)

        self.params['w_4'] = scale * \
            np.random.randn(int(3 * 3 * num_filters_3), num_classes)
        self.params['b_4'] = np.zeros(num_classes)
    ...
    def loss(self, x, y):
        ...
        w_4, b_4 = self.params['w_4'], self.params['b_4']
        ...
        conv_3, cache_3 = conv_relu_pool(
            conv_2, w_3, b_3, conv_param, pool_param)
        scores, linear_cache = linear(conv_3, w_4, b_4)
        ...
        dx_4, grads['w_4'], grads['b_4'] = dlinear(dout, linear_cache)
        dx_3, grads['w_3'], grads['b_3'] = dconv_relu_pool(dx_4, cache_3)
        dx_2, grads['w_2'], grads['b_2'] = dconv_relu_pool(dx_3, cache_2)
        dx_1, grads['w_1'], grads['b_1'] = dconv_relu_pool(dx_2, cache_1)

        grads['w_4'] += self.reg * self.params['w_4']
    ...
    def train(self, x, y, x_val, y_val, batch_size=200, lr=1e-3, reg=1e-5, epoch=3):
        ...
        for i in range(epoch):
            ...
            for iter in range(iter_per_epoch):
                ...
                writer.add_scalars('three_layer_loss', {
                                   'train': train_loss, 'val': val_loss}, iter)

                # SGD
                self.params['w_4'] -= lr * grads['w_4']
                self.params['b_4'] -= lr * grads['b_4']
                ...
                writer.add_scalars('three_layer_accuracy', {
    ...
    def predict(self, x):
        ...
        conv_3, _ = conv_relu_pool(
            conv_2, params['w_3'], params['b_3'], conv_param, pool_param)
        scores, _ = linear(conv_3, params['w_4'], params['b_4'])
